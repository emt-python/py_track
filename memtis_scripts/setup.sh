...
function func_memtis_setting() {
    ...
    if [[ "x${CONFIG_CXL_MODE}" == "xon" ]]; then
        ${DIR}/memtis_scripts/set_uncore_freq.sh on
        echo "enabled" | tee /sys/kernel/mm/htmm/htmm_cxl_mode
    else
        ${DIR}/memtis_scripts/set_uncore_freq.sh off
...
function func_prepare() {
    echo "Preparing memtis environment..."
...
function func_main() {
    ${DIR}/memtis_scripts/kill_ksampled
    TIME="/usr/bin/time"
    ...
    # set memcg for htmm
    sudo ${DIR}/memtis_scripts/set_htmm_memcg.sh htmm remove
    sudo ${DIR}/memtis_scripts/set_htmm_memcg.sh htmm $RUN_BENCH_PID enable
    # sudo ${DIR}/memtis_scripts/set_mem_size.sh htmm 0 ${BENCH_DRAM} # do this later in run_bench.sh
    sleep 2
