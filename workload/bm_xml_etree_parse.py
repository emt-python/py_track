...
def build_xml_tree_v2(etree):
    ...
    for c in range(2000):  # Increased from 50 to 500
        ...
        for i in range(500):  # Increased from 100 to 500
            ...
            subchild.text = f'LEAF-{c}-{i}'
            # Adding more attributes to consume more memory
            subchild.set('attr1', 'value1')
            subchild.set('attr2', 'value2')
    ...
    for i in range(500):  # Making the deep tree deeper, from 50 to 100 levels
        ...
        for j in range(10):  # Adding more elements at each level of the deep subtree
            intermediate_leaf = SubElement(
                current_level, 'intermediate', tag_type="intermediateleaf")
            intermediate_leaf.text = f'Intermediate-{i}-{j}'
    SubElement(current_level, 'deepleaf',
               tag_type="deepestleaf").text = "DEEPEST LEAF"

    # Storing the number of elements for later and increasing attributes
    nb_elems = sum(1 for elem in root.iter())
    root.set('nb-elems', str(nb_elems))
    root.set('info', 'This is a more memory-intensive XML tree')
...
def bench_etree(iterations, etree, bench_func):
    xml_root = build_xml_tree_v2(etree)
...
BENCHMARKS = 'parse'.split()
# parse iterparse generate process
